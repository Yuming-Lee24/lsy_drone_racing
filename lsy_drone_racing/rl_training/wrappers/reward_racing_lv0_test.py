...
class RacingRewardWrapper(VectorWrapper):
    ...
    def _compute_reward(
        ...
        p_ground = 0.1 * np.where(on_ground, 0.1, 0.0)
    ...
    def get_rollout_stats(self):
        """获取本轮 rollout 中完成的 episode 的平均奖励。"""
        if len(self._finished_episodes["total"]) == 0:
            return None
        
        stats = {k: np.mean(v) for k, v in self._finished_episodes.items()}
        stats["n_episodes"] = len(self._finished_episodes["total"])
    ...
    def _compute_align_reward(self, obs: dict) -> NDArray:
            ...
            # 只有速度大于 0.5m/s 才给对齐奖励，防止静止时数值不稳定
            valid_mask = (vel_norm.squeeze() > 0.5)
            
            vel_dir = vel / np.maximum(vel_norm, 1e-6)
            
            # 2. 门法向量
            safe_idx = np.clip(target_gate, 0, self.n_gates - 1)
            batch_idx = np.arange(self.num_envs)
            gate_quat = gates_quat[batch_idx, safe_idx]
            
            gate_rot = Rotation.from_quat(gate_quat)
            gate_normal = gate_rot.apply(self.GATE_NORMAL_LOCAL)
            
            # 3. 点积
            align = np.sum(vel_dir * gate_normal, axis=1)
            
            # 如果已经完赛，对齐奖励为0
            finished = (target_gate == -1)
            
            return np.where(valid_mask & ~finished, align, 0.0)
